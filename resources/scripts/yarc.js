    ...
    if(yCore.activePlayer != -1 && yRemote.panelVisible){ //only run if footer visible and and a player is active
      ...
      yCore.sendJsonRPC(
        ...
        '[{"jsonrpc":"2.0","method":"Application.GetProperties","id":1,"params":[["muted"]]},'
          ...
          + yCore.activePlayer
          + ', "properties": [ "title", "showtitle", "artist", "thumbnail", "streamdetails", "file",' 
        + '"season", "episode"] }, "id": 3 }]',
    ...
    stop: function( event, ui ) {
      ...
      yCore.sendJsonRPC(
        ...
        '{"jsonrpc":"2.0","id":1,"method":"Player.Seek","params":{"playerid":' + yCore.activePlayer 
          + ',"value":{"hours": ' + yRemote.seekTime[0] + ', "minutes": ' + yRemote.seekTime[1] 
 ...
 * There are also the tools with shutdown/sleep etc, device orientation on/off switch, instruciton video 
  ...
  init: function(){
    if (!yStart.initDone){  //that it doesn't run twice
      ...
      yCore.sendJsonRPC(
          ...
          function(resultgetRecentMovies){
            ...
            if(resultgetRecentMovies["result"]["limits"]["end"] == 0){
              ...
              $(".ui-grid-d > .ui-block-a, .ui-grid-d > .ui-block-b, .ui-grid-d > .ui-block-c, .ui-grid-d > .ui-block-d, "
                  + ".ui-grid-d > .ui-block-e").css("width", (100 / yCore.headerMenuItems) + "%");
            } else {
        ...
        if(addonopenslist.length == 0){
            setTimeout(function(){$("#topAddons").append($.t("no-matching")).trigger("create");}, 1500);
        }

        //inject link to all favorites
        $("#topFavs").append(
        ...
        yCore.sendJsonRPC(
            ...
            function(resultgetTop10Favs){
                if(resultgetTop10Favs["result"]["limits"]["end"] == 0){
                    setTimeout(function(){$("#topFavs").append($.t("no-matching")).trigger("create");}, 1500);
                } else {
                    for (var i = 0; (i < 9 && i < resultgetTop10Favs["result"]["limits"]["end"]); i++) {
        ...
        yStart.initDone = 1;
        setTimeout(function(){$("#yarcLoading").fadeOut("slow")}, 1000);
    }
...
var yMovies = {
  ...
  init: function() {
    ...
    $("body").delegate("#toggleMovieSeen", "click", function(e){
        yCore.sendJsonRPC(
            'toggleMovieSeen',
            '{"jsonrpc":"2.0","method":"VideoLibrary.SetMovieDetails","params":{"movieid":' + yMovies.moviesJSON["result"]["movies"][$("#toggleMovieSeen").attr('data-yMovieId')]["movieid"] + ',"playcount":1},"id":1}',
            function(){
    ...
    $("body").delegate("#toggleMovieUnSeen", "click", function(e){
        yCore.sendJsonRPC(
            'toggleMovieUnSeen',
            '{"jsonrpc":"2.0","method":"VideoLibrary.SetMovieDetails","params":{"movieid":' + yMovies.moviesJSON["result"]["movies"][$("#toggleMovieUnSeen").attr('data-yMovieId')]["movieid"] + ',"playcount":0},"id":1}',
            function(){
  ...
  openMovieItem: function(movieNr) {
    ...
    yCore.sendJsonRPC(
        'GetMovieDetails',
        '{"jsonrpc":"2.0","method":"VideoLibrary.GetMovieDetails","params":[' + yMovies.moviesJSON["result"]["movies"][movieNr]["movieid"] + ',["title",'
            + '"thumbnail","playcount","resume","year","file","runtime","streamdetails","plot","trailer"]],"id":"1"}',
            ...
            if(!yS.yS.hideLanguageMovies){ 
                ...
                for (var j=0;  j < movieDetails["streamdetails"]["audio"].length; j++){//run whole kodi-language list
                    if(movieDetails["streamdetails"]["audio"][j]["language"] in langCodeToDescFlag){//if code is in json
                        ...
                        movieDetails["streamdetails"]["audio"][j]["isocode"] = langCodeToDescFlag[lang]["iso639-2"];
                    }
                }
                ...
                */
                        ...
                        if (langCodeToDescFlag.hasOwnProperty(code)) {
                            if (movieDetails["file"].toLowerCase().indexOf("[" + code + "]") >= 0) {//if code is found in filename
                                ...
                                for (var j=0;  j < movieDetails["streamdetails"]["audio"].length; j++){
                                    ...
                                    if(langCodeToDescFlag[code]["iso639-2"] == movieDetails["streamdetails"]["audio"][j]["isocode"]){
                ...
                for (var j=0;  j < movieDetails["streamdetails"]["subtitle"].length; j++){//run whole kodi-language list
                    ...
                    movieDetails["streamdetails"]["subtitle"][j]["isocode"] = langCodeToDescFlag[langSub]["iso639-2"];
                    }
                }
                        ...
                        if (langCodeToDescFlag.hasOwnProperty(code)) {
                            if (movieDetails["file"].toLowerCase().indexOf("[sub:" + code + "]") >= 0) {//if code is found in filename
                                ...
                                for (var j=0;  j < movieDetails["streamdetails"]["subtitle"].length; j++){
                                    ...
                                    if(langCodeToDescFlag[code]["iso639-2"] == movieDetails["streamdetails"]["subtitle"][j]["isocode"]){
                                    codeIsSet = true;//... remeber it to...
                                    }
                                }
...
var ySeries = {
  ...
  openSeries: function(TvShowId){
    ...
    yCore.sendJsonRPC(
      'GetSeasons',
      '{"jsonrpc": "2.0", "method": "VideoLibrary.GetSeasons", "params": {"properties": ["season", "showtitle", "thumbnail", "playcount"], "tvshowid":'
                  + TvShowId + '}, "id": 1}',
        ...
        for (var j = 0; j < resultGetSeasons["result"]["limits"]["end"]; j++) {
            ...
            if(!yS.yS.hidePrevPics){
                imagePoster = yTools.imageUrlNormalizer(
                                resultGetSeasons["result"]["seasons"][j]["thumbnail"],
                                "?",
                                "tag",
                                "seriesPoster",
                                ""
            ...
            $("#"+TvShowId).append(
                "<div data-role='collapsible' class='openSeason' data-yTVShowID='" + TvShowId 
  ...
  /*
   * called if a season is opened
   */
  openSeason: function(TvShowId, TVShowSeasonID){
  ...
  showEpisodeDetails: function(episodeID){
    ...
    yCore.sendJsonRPC(
      'GetEpisodeDetails',
      '{"jsonrpc": "2.0", "method": "VideoLibrary.GetEpisodeDetails", "params": { "properties": ["season","episode", "showtitle", "plot", "fanart", "thumbnail", "file", "rating", "playcount", "streamdetails", "resume","firstaired","runtime"],"episodeid":' + episodeID + '}, "id": 1}',
          ...
          */
          ...
          for (var j=0;  j < episodeDetails["streamdetails"]["subtitle"].length; j++){//run whole kodi-language list
              if(episodeDetails["streamdetails"]["subtitle"][j]["language"] in langCodeToDescFlag){//if code is in json
                  ...
                  episodeDetails["streamdetails"]["subtitle"][j]["isocode"] = langCodeToDescFlag[langSub]["iso639-2"];
              }
          }
                  ...
                  if (langCodeToDescFlag.hasOwnProperty(code)) {
                      if (episodeDetails["file"].toLowerCase().indexOf("[sub:" + code + "]") >= 0) {//if code is found in filename
                          ...
                          for (var j=0;  j < episodeDetails["streamdetails"]["subtitle"].length; j++){
                              //if code is already in streamdetails-subtitle...
                              if(langCodeToDescFlag[code]["iso639-2"] == episodeDetails["streamdetails"]["subtitle"][j]["isocode"]){
                              codeIsSet = true;//... remeber it to...
                              }
                          }
                          if(!codeIsSet){//..not add it again to aopton list
      ...
      yCore.sendJsonRPC(
        ...
        function(resultGetDeactivatedAddons){
          for (var i = 0; i < (resultGetDeactivatedAddons["result"]["limits"]["end"]); i++) {
            if (yS.yS.addons.hasOwnProperty(resultGetDeactivatedAddons["result"]["addons"][i]["addonid"])) {
              delete yS.yS.addons[resultGetDeactivatedAddons["result"]["addons"][i]["addonid"]];
            }
          }
    ...
    $( "#addonDetailsList" ).on( "swiperight",  function( e ) {
      if (e.type === "swiperight") {
        ...
        if(yAddons.addonBackPath.length == 2 && $("#back").attr('data-yAddonIsBack') == "back"){
          ...
          $("#addonDetailsList").empty();
          yAddons.addonBackPath.pop();yAddons.addonBackPath.pop();
          yAddons.openKodiFavs(backFilepath, backFanartPath);
        } else {
          $("#addonDetailsList").empty();
          yAddons.addonBackPath.pop();yAddons.addonBackPath.pop();
...
if (localStorage.getItem("yarcSettings") === null) {
  //check if there are already settings, if not, set Kodi language if in yarc avialbe or english, has to be done befor interface begins to load
  yCore.sendJsonRPC(
    'GetLanguage',
